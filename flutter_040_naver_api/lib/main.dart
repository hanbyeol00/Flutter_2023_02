...
class _MyHomePageState extends State<MyHomePage> {
  /*
   * resultMovie 는 변수 선언 이지만
   * 실제적으로 함수로서의 역활을 수행한다
   * 
   * FutureBuilder 의 future 속성에 변수를 설정하고
   * FutureBuilder 가 화면에 rendering 을 하기 전엔
   * resultMovie 변수를 참조(이때는 함수처럼 작동한다)
   * 변수를 참조하지만 NaverApi().loadMovie() 함수를 실행하는것과 같다
   */
  Future<List<Movie>?> resultMovie = NaverApi().loadMovie();

  // input box 를 핸들링 하기 위한 ref id 를 선언하기
  ...
  Widget build(BuildContext context) {
    return Scaffold(
       ...
       * 2. 가져온 데이터는 builder 속성의 callback 함수에 snapshot 매개변수로 전달한다
       * 3. ListView.builder 에게 데이터를 전달한다
       * 4. ListView.builder 는 snapshot.data 에 담긴 데이터(보통 list)를
       *        화면에 rendering 한다
       * 5. ListView.builder 는 itemBuilder 속성에 설정된 callback 함수를 사용하여
       *        각각 개별적인 데이터를 화면에 rendering 한다
       ...
       *          다시 FutureBuilder 에게 통보하여 데이터를 그릴 수 있는
       *          개수 만큼만 가져오도록 알려준다
